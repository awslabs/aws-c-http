...
DECODER_TEST_CASE(websocket_decoder_close_frame_status_code) {
    ...
    /* The 2-byte status code may be split across chunks, so feed one byte at a time too */
    const uint16_t split_codes[] = {1000 /* legal */, 1005 /* illegal */};
    for (size_t i = 0; i < AWS_ARRAY_SIZE(split_codes); ++i) {
        s_decoder_tester_reset(&tester);
        uint8_t input[] = {0x88, 0x02, (uint8_t)(split_codes[i] >> 8), (uint8_t)(split_codes[i] & 0xFF)};
        int result = AWS_OP_SUCCESS;
        for (size_t j = 0; j < AWS_ARRAY_SIZE(input) && result == AWS_OP_SUCCESS; ++j) {
            struct aws_byte_cursor one_byte = aws_byte_cursor_from_array(&input[j], 1);
            result = aws_websocket_decoder_process(&tester.decoder, &one_byte, &frame_complete);
        }
        if (i == 0) {
