...
tar -xvzf nginx-1.21.6.tar.gz && rm nginx-1.21.6.tar.gz
...
cp localhost.key localhost.crt ./nginx/conf/
# copy the njs script
cp -r ./tests/localhost/njs ./nginx/conf/
# overwrite the njs configuration
cp ./tests/localhost/nginx.conf ./nginx/conf/
