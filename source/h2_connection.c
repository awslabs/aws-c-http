...
static struct aws_h2_err s_decoder_on_headers_end(
    ...
    enum aws_http_header_block block_type,
    void *userdata);
static struct aws_h2_err s_decoder_on_push_promise(uint32_t stream_id, uint32_t promised_stream_id, void *userdata);
...
static struct aws_h2_err s_decoder_on_rst_stream(uint32_t stream_id, uint32_t h2_error_code, void *userdata);
...
static struct aws_h2_err s_decoder_on_window_update(uint32_t stream_id, uint32_t window_size_increment, void *userdata);
 ...
 * Returns failed aws_h2_err if it is a connection error to receive this frame.
 */
struct aws_h2_err s_get_active_stream_for_incoming_frame(
    struct aws_h2_connection *connection,
    uint32_t stream_id,
    ...
    *out_stream = NULL;
    ...
    return aws_h2_err_from_h2_code(AWS_H2_ERR_STREAM_CLOSED);
}

/* Decoder callbacks */

struct aws_h2_err s_decoder_on_headers_begin(uint32_t stream_id, void *userdata) {
...
struct aws_h2_err s_decoder_on_headers_i(
    uint32_t stream_id,
    const struct aws_http_header *header,
    enum aws_http_header_name name_enum,
    enum aws_http_header_block block_type,
...
struct aws_h2_err s_decoder_on_headers_end(
    uint32_t stream_id,
    bool malformed,
    enum aws_http_header_block block_type,
    void *userdata) {
...
struct aws_h2_err s_decoder_on_push_promise(uint32_t stream_id, uint32_t promised_stream_id, void *userdata) {
    struct aws_h2_connection *connection = userdata;
...
struct aws_h2_err s_decoder_on_data(uint32_t stream_id, struct aws_byte_cursor data, void *userdata) {
    struct aws_h2_connection *connection = userdata;
    ...
    /* Pass data to stream */
    struct aws_h2_stream *stream;
    struct aws_h2_err err = s_get_active_stream_for_incoming_frame(connection, stream_id, AWS_H2_FRAME_T_DATA, &stream);
    if (aws_h2_err_failed(err)) {
        return err;
    }
...
struct aws_h2_err s_decoder_on_end_stream(uint32_t stream_id, void *userdata) {
    struct aws_h2_connection *connection = userdata;
    ...
    if (found) {
        ...
        struct aws_h2_err err = aws_h2_stream_on_decoder_end_stream(stream);
        if (aws_h2_err_failed(err)) {
            return err;
        }
    }
...
static int s_handler_process_read_message(
    ...
    struct aws_h2_err err = aws_h2_decode(connection->thread_data.decoder, &message_cursor);
    if (aws_h2_err_failed(err)) {
        /* #TODO: send GOAWAY as a result of this aws_h2_err trickling all the way up to the top of the stack */
