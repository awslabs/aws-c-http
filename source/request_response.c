...
int aws_http2_stream_write_data(
    ...
    /* aws_http2_stream_write_data_options is now a typedef of aws_http_stream_write_data_options */
    return aws_http_stream_write_data(http2_stream, (struct aws_http_stream_write_data_options *)options);
}
int aws_http_stream_write_data(
    struct aws_http_stream *http_stream,
    const struct aws_http_stream_write_data_options *options) {
    AWS_PRECONDITION(http_stream);
    AWS_PRECONDITION(http_stream->vtable);
    AWS_PRECONDITION(http_stream->vtable->write_data);
    AWS_PRECONDITION(options);
    return http_stream->vtable->write_data(http_stream, options);
}
