...
void aws_hpack_encoder_init(
    ...
    aws_hpack_dynamic_table_init(&encoder->dynamic_table, allocator, AWS_LS_HTTP_ENCODER, log_id);

    encoder->dynamic_table_size_update.pending = false;
    encoder->dynamic_table_size_update.last_value = SIZE_MAX;
...
void aws_hpack_encoder_clean_up(struct aws_hpack_encoder *encoder) {
    aws_hpack_dynamic_table_clean_up(&encoder->dynamic_table);
    AWS_ZERO_STRUCT(*encoder);
}
...
size_t aws_hpack_encoder_find_index(
    const struct aws_hpack_encoder *encoder,
    const struct aws_http_header *header,
    bool search_value,
    bool *out_found_value) {

    size_t index;

    if (search_value) {
        /* TODO: we can probably remove the `out_found_value` arg from the find_name_and_value() functions.
         * I mean, we're searching by name and value, so we ALWAYS set this true if the function succeeds?
         * Leaving logic as-is for now, but if tests all pass lets remove it */

        /* Check name-and-value first in static table */
        index = aws_hpack_static_table_find_name_and_value(header, out_found_value);
        if (index != 0) {
            return index;
        }
        /* Check name-and-value in dynamic table */
        index = aws_hpack_dynamic_table_find_name_only(header, out_found_value);
        if (index != 0) {
            return index;
        }
    }
    ...
    index = aws_hpack_static_table_find_name_only(header->name);
    if (index != 0) {
        *out_found_value = false;
        return index;
    }
    index = aws_hpack_dynamic_table_find_name_only(header->name);
    if (index != 0) {
        *out_found_value = false;
        return index;
    }
...
void aws_hpack_encoder_update_max_table_size(struct aws_hpack_encoder *encoder, uint32_t new_max_size) {
    ...
    encoder->dynamic_table_size_update.last_value = new_max_size;
}
...
static int s_encode_header_field(
    ...
    size_t header_index = aws_hpack_encoder_find_index(encoder, header, true, &found_indexed_value);
    ...
    if (AWS_HPACK_ENTRY_LITERAL_HEADER_FIELD_WITH_INCREMENTAL_INDEXING == literal_entry_type) {
        if (aws_hpack_dynamic_table_insert_header(&encoder->dynamic_table, header)) {
            goto error;
        }
    }
...
int aws_hpack_encode_header_block(
    ...
    if (encoder->dynamic_table_size_update.pending) {
        if (encoder->dynamic_table_size_update.smallest_value != encoder->dynamic_table_size_update.last_value) {
            ...
            if (aws_hpack_dynamic_table_resize(&encoder->dynamic_table, smallest_update_value)) {
        ...
        size_t last_update_value = encoder->dynamic_table_size_update.last_value;
        HPACK_LOGF(TRACE, encoder, "Encoding last dynamic table size update entry size: %zu", last_update_value);
        if (aws_hpack_dynamic_table_resize(&encoder->dynamic_table, last_update_value)) {
