...
static int s_stream_send_response(struct aws_http_stream *stream, struct aws_http_message *response) {
    ...
    bool expects_no_body = h1_stream->base.request_method == AWS_HTTP_METHOD_HEAD;
    err = aws_h1_encoder_message_init_from_response(&encoder_message, stream->alloc, response, expects_no_body);
    if (err) {
...
static int s_mark_head_done(struct aws_h1_stream *incoming_stream) {
    ...
    if (!aws_h1_decoder_get_expects_no_body(connection->thread_data.incoming_stream_decoder)) {
...
static int s_handler_process_read_message(
    ...
    while (message_cursor.len > 0) {
        ...
        if (connection->thread_data.has_switched_protocols) {
            ...
            bool expects_no_body = connection->thread_data.incoming_stream->base.request_method == AWS_HTTP_METHOD_HEAD;
            aws_h1_decoder_set_expects_no_body(connection->thread_data.incoming_stream_decoder, expects_no_body);
