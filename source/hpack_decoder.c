...
void aws_hpack_decoder_init(struct aws_hpack_decoder *decoder, struct aws_allocator *allocator, const void *log_id) {
    ...
    aws_hpack_dynamic_table_init(&decoder->dynamic_table, allocator, AWS_LS_HTTP_DECODER, log_id);
}

void aws_hpack_decoder_clean_up(struct aws_hpack_decoder *decoder) {
    aws_hpack_dynamic_table_clean_up(&decoder->dynamic_table);
    aws_byte_buf_clean_up(&decoder->progress_entry.scratch);
    AWS_ZERO_STRUCT(*decoder);
}

const struct aws_http_header *aws_hpack_decoder_get_header(const struct aws_hpack_decoder *decoder, size_t index) {
    if (index == 0 || index >= aws_hpack_static_table_num_elements + decoder->dynamic_table.num_elements) {
        aws_raise_error(AWS_ERROR_INVALID_INDEX);
        return NULL;
    }

    /* Check static table */
    if (index < aws_hpack_static_table_num_elements) {
        return &s_static_header_table[index];
    }

    /* Check dynamic table */
    return s_dynamic_table_get(context, index - aws_hpack_static_table_num_elements);
}
...
static const struct aws_http_header *s_get_header_u64(const struct aws_hpack_decoder *decoder, uint64_t index) {
    ...
    return aws_hpack_decoder_get_header(decoder, (size_t)index);
}

void aws_hpack_decoder_update_max_table_size(struct aws_hpack_decoder *decoder, uint32_t setting_max_size) {
    decoder->dynamic_table.protocol_max_size_setting = setting_max_size;
}
        ...
        switch (decoder->progress_entry.state) {
            ...
            case HPACK_ENTRY_STATE_LITERAL_VALUE_STRING: {
                ...
                if (literal->compression == AWS_HTTP_HEADER_COMPRESSION_USE_CACHE) {
                    if (aws_hpack_dynamic_table_insert_header(&decoder->dynamic_table, &header)) {
                        return AWS_OP_ERR;
                    }
                }
            ...
            case HPACK_ENTRY_STATE_DYNAMIC_TABLE_RESIZE: {
                ...
                if (*size64 > decoder->dynamic_table.protocol_max_size_setting) {
                ...
                if (aws_hpack_dynamic_table_resize(&decoder->dynamic_table, size)) {
                    return AWS_OP_ERR;
                }
