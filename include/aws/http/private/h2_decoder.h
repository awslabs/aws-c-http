...
/* Return a failed aws_h2_err from any callback to stop the decoder and cause a Connection Error */
struct aws_h2_decoder_vtable {
    ...
    struct aws_h2_err (*on_headers_begin)(uint32_t stream_id, void *userdata);
    struct aws_h2_err (*on_headers_i)(
        uint32_t stream_id,
     ...
     * No other decoder callbacks will occur in this time.
     * If something is malformed, no further _i() calls occur, and it is reported in _end() */
    struct aws_h2_err (*on_push_promise_begin)(uint32_t stream_id, uint32_t promised_stream_id, void *userdata);
    struct aws_h2_err (*on_push_promise_i)(
        uint32_t stream_id,
        const struct aws_http_header *header,
        enum aws_http_header_name name_enum,
        void *userdata);
    struct aws_h2_err (*on_push_promise_end)(uint32_t stream_id, bool malformed, void *userdata);

    /* Called repeatedly as DATA frames are processed.
     * This may fire multiple times per actual DATA frame. */
    struct aws_h2_err (*on_data)(uint32_t stream_id, struct aws_byte_cursor data, void *userdata);
    ...
    struct aws_h2_err (*on_end_stream)(uint32_t stream_id, void *userdata);

    /* Called once for RST_STREAM frame */
    struct aws_h2_err (*on_rst_stream)(uint32_t stream_id, uint32_t error_code, void *userdata);

    /* Called once For PING frame with ACK flag set */
    struct aws_h2_err (*on_ping_ack)(uint8_t opaque_data[AWS_H2_PING_DATA_SIZE], void *userdata);

    /* Called once for PING frame (no ACK flag set)*/
    struct aws_h2_err (*on_ping)(uint8_t opaque_data[AWS_H2_PING_DATA_SIZE], void *userdata);

    /* Called once for SETTINGS frame with ACK flag */
    struct aws_h2_err (*on_settings_ack)(void *userdata);
    ...
    struct aws_h2_err (*on_goaway_i)(struct aws_byte_cursor debug_data, void *userdata);
    struct aws_h2_err (*on_goaway_end)(void *userdata);

    /* Called once for WINDOW_UPDATE frame */
    struct aws_h2_err (*on_window_update)(uint32_t stream_id, uint32_t window_size_increment, void *userdata);
};
...
/* If failed aws_h2_err returned, it is a Connection Error */
AWS_HTTP_API struct aws_h2_err aws_h2_decode(struct aws_h2_decoder *decoder, struct aws_byte_cursor *data);
