...
struct aws_h2_connection {
    ...
    struct {
        ...
        /**
         * Contains struct aws_http_stream *
         * The streams that encoded the last frame between it's encoded to the `aws_io_message` to the
         *`aws_io_message.on_completion` invoked.
         **/
        struct aws_array_list finish_encoding_streams_list;
    } thread_data;
