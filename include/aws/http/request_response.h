 ...
 * If error_code is AWS_ERROR_SUCCESS (0), the data was successfully sent.
 * Any other error_code indicates that the HTTP-stream is in the process of terminating.
 * If the error_code is AWS_ERROR_HTTP_STREAM_HAS_COMPLETED, the stream's termination
 * has nothing to do with this chunk, but any other non-zero error code indicates
