 ...
 * If error_code is AWS_ERROR_SUCCESS, the data was successfully sent.
 * If error_code is AWS_ERROR_HTTP_STREAM_HAS_COMPLETED, the stream terminated before the chunk could be fully sent.
 * Any other error_code indicates a problem with this chunk's data.
 * This is always invoked on the HTTP connection's event-loop thread.
 */
typedef void aws_http1_stream_write_chunk_complete_fn(int error_code, void *user_data);
